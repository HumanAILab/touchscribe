...
def display_image():
    ...
    while True:
        ...
        if key == 27:  # ESC
            break
        global mode
        _number, _mode = select_mode(key, mode)
...
def logging_csv(number, mode, landmark_list, point_history_list):
    ...
    if mode == 2 and (0 <= number <= 9):
        csv_path = 'model/point_history_classifier/'+finger+'_point_history.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
...
def process_server_visual_data(data):
    ...
    image = base64_to_cv2_image(frame_base64string)
    ...
    if results.multi_hand_landmarks is not None:
        for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
            ...
            # if hand_sign_id == 0 or hand_sign_id == 1:  # Point gesture
            #     print("adding points")
            ...
            if hand_sign_id == 0 or hand_sign_id == 1 or hand_sign_id == 4 or hand_sign_id == 5:  # Point gesture
                print(f"adding {finger} index points")
                if finger == "thumb": point_history.append(landmark_list[4])
                elif finger == "index": point_history.append(landmark_list[8])
            else:
...
if __name__ == '__main__':
    ...
    finger = "thumb"
    point_history_classifier = PointHistoryClassifier(model_path=finger+"_point_history_classifier.tflite")
    ...
    with open(
            'model/point_history_classifier/'+finger+'_point_classifier_label.csv',
            encoding='utf-8-sig') as f:
    ...
    packet_tail = "_TAIL".encode('utf-8') 
